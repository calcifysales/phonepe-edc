/**
 * Robust Standalone QR Code Generator (Kazuhiko Arase)
 */
(function (global) {
  ...
  // High-level render helper
  global.createQRCodeCanvas = function (text, size, options) {
    size = size || 220;
    options = options || {};
    var colorDark = options.colorDark || "#1e1b4b";
    ...
    var qr = new QRCode(0, correctLevel);
    qr.addData(text);
    qr.make();
    ...
    var margin = options.margin !== undefined ? options.margin : 16;
    var availableSize = size - (margin * 2);
    var cellSize = availableSize / moduleCount;
  ...
  // Universal QRCode Constructor Wrapper
  function QRCodeWrapper(target, options) {
    if (typeof target === 'number') {
      return new QRCode(target, options);
    }
    var opt = {};
    ...
    var size = opt.width || opt.height || 220;
    var text = opt.text || '';
    var el = typeof target === 'string' ? document.getElementById(target) : target;

    function render(str) {
      var canvas = global.createQRCodeCanvas(str, size, opt);
      if (el) {
        el.innerHTML = '';
  ...
  QRCodeWrapper.CorrectLevel = QRErrorCorrectLevel;
  global.QRCode = QRCodeWrapper;
